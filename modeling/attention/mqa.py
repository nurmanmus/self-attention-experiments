...
class RopelessMQA(torch.nn.Module):
    ...
    def __init__(self, d_model, n_heads):
        ...
        self.wq = torch.nn.Parameter(0.02*torch.randn((d_model, d_model)))
        self.wkv = torch.nn.Parameter(0.02*torch.randn((d_model, 2 * (d_model//n_heads))))
...
class Rope_MQA(torch.nn.Module):
    ...
    def __init__(self, d_model, n_heads, max_len=1024, rope_theta=10000.0):
        ...
        self.wq = torch.nn.Parameter(0.02*torch.randn((d_model, d_model)))
        self.wkv = torch.nn.Parameter(0.02*torch.randn((d_model, 2 * self.dh)))
        self.wo = torch.nn.Parameter(0.02*torch.randn((d_model, d_model)))
